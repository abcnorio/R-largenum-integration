...
# log parallel
sprintf("%.16e", exp(simpsonrule.nlb(fx=f.log, lower=lower, upper=upper, method="log",Nsteps=Nsteps, parallel=TRUE)) )
...
sprintf("%.16e", as.numeric(simpsonrule.nlb(fx=f2.brob, lower=lower, upper=upper, method="brob",Nsteps=Nsteps)) )
# via integrate
...
micro.res <- microbenchmark( simpsonrule.nlb(fx=f, lower=lower, upper=upper, method="normal",Nsteps=Nsteps) ,
			   exp(simpsonrule.nlb(fx=f.log, lower=lower, upper=upper, method="log",Nsteps=Nsteps)) ,
			   exp(simpsonrule.nlb(fx=f.log, lower=lower, upper=upper, method="log",Nsteps=Nsteps, parallel=TRUE)) ,
			   as.numeric(simpsonrule.nlb(fx=f.brob, lower=lower, upper=upper, method="brob",Nsteps=Nsteps)) ,
			   as.numeric(simpsonrule.nlb(fx=f2.brob, lower=lower, upper=upper, method="brob",Nsteps=Nsteps)) ,
